...
class Cfg(StrategyConfig,frozen=True):
 ...
 entry_mode:str='proxy'; entry_clock_ms:int=0; ts_ticks_per_bar:int=3; ts_fast:int=3; ts_slow:int=5; ts_conf1:int=8; ts_conf2:int=13; ts_cross_only:bool=False; ts_use_macd:bool=False; ts_include_conf:bool=True
 ...
 def on_quote_tick(self,t):
  ...
  if not self.entries:
   ...
   if self.config.session_start_hour<=h<self.config.session_end_hour:
    if self.config.entry_clock_ms>0 and self.last_entry_eval_ns and (self.now_ns-self.last_entry_eval_ns)<self.config.entry_clock_ms*1_000_000:return
    self.last_entry_eval_ns=self.now_ns
