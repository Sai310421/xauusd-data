...
def build_geom(bars, lookback=48):
    ...
    w = bars[-lookback:]
    hi = max(range(len(w)), key=lambda i: w[i].h)
    lo = min(range(len(w)), key=lambda i: w[i].l)
    H, L = w[hi].h, w[lo].l
    if hi > lo:
        side, A, B = -1, L, H
    else:
    ...
    sl = B - side * 0.20 * wave
    tp1 = B + side * 1.0 * wave
    tp2 = B + side * 1.272 * wave
    tp3 = B + side * 1.618 * wave
    return side, A, B, wave, entry, sl, tp1, tp2, tp3

def regime(bars):
    a14, a50 = atr(bars, 14), atr(bars, 50)
    if not a14 or not a50: return 'NORMAL', a14
    closes = [b.c for b in bars]
    e1 = ema(closes[-20:], 9)
    e6 = ema(closes[:-5][-20:], 9) if len(closes) >= 14 else e1
...
def summarize(rows):
    ...
    return {
        'N': n, 'WR_pct': wr, 'PF': pf,
        'expectancy': float(np.mean(pn)) if pn else 0.0,
        'net': float(sum(pn)), 'maxDD_pct': dd,
        'avg_mfe': float(np.mean([r['mfe'] for r in rows])) if rows else 0.0,
        'avg_mae': float(np.mean([r['mae'] for r in rows])) if rows else 0.0,
        'pyr_added_trades': sum(bool(r.get('pyr_added')) for r in rows),
        'stdv_added_trades': sum(bool(r.get('stdv_added')) for r in rows),
        'eod_force_closed_trades': sum(bool(r.get('eod_force_closed')) for r in rows),
    }

def _finish(active, rows, eod=False):
    rows.append({
        'pnl': sum(z['pnl'] for z in active['legs']),
        'mfe': active['mfe'], 'mae': active['mae'],
        'pyr_added': active['pyr_added'], 'stdv_added': active['stdv_added'],
        'eod_force_closed': eod,
        'legs': [{'kind': z['kind'], 'pnl': z['pnl']} for z in active['legs']],
    })

def simulate(ticks, start, end, mode, stdv_target, stdv_frac, stdv_stop):
    bars=[]; cur=None; rows=[]; active=None; last_signal_bucket=None; last_t=None
    for i in range(start, end):
        t=ticks[i]; last_t=t; ts=int(t.ts_event); m=mid(t); bkt=bucket(ts)
        ...
        if active:
            ...
            for leg in active['legs']:
                ...
                if hit_sl or hit_tp:
                    ex=leg['sl'] if hit_sl else leg['tp']
                    leg['pnl']=(ex-leg['entry'])*side*leg['w']; leg['closed']=True
                ...
                if a14:
                    ...
                    for leg in active['legs']:
                        if leg['kind']=='ATR' and not leg['closed']:
                            ns=x-side*k*a14
                            leg['sl']=max(leg['sl'],ns) if side>0 else min(leg['sl'],ns)

            if mode in ('PYR','STDV') and not active['pyr_added']:
                f1=next(z for z in active['legs'] if z['kind']=='F1')
                ar=next(z for z in active['legs'] if z['kind']=='ATR')
                if f1['closed'] and not ar['closed'] and (x-ar['entry'])*side>0 and (ar['sl']-ar['entry'])*side>=0:
                    rg,a14=regime(bars)
                    if a14:
                        en=px_entry(t,side)
                        active['legs'].append({'kind':'PYR','entry':en,'w':0.45,'sl':en-side*0.75*a14,'tp':None,'closed':False,'pnl':0.0})
                        active['pyr_added']=True

            if mode=='STDV' and not active['stdv_added']:
                f2=next(z for z in active['legs'] if z['kind']=='F2')
                ar=next(z for z in active['legs'] if z['kind']=='ATR')
                g=build_geom(bars)
                if f2['closed'] and not ar['closed'] and (x-ar['entry'])*side>0 and g and g[0]==side:
                    _,_,B,wave,_,_,tp1,_,_=g
                    target=B+side*stdv_target*wave
                    ssl=B+side*stdv_stop*wave
                    en=px_entry(t,side)
                    if (target-en)*side>0:
                        if (en-ssl)*side<=0: ssl=tp1
                        active['legs'].append({'kind':'STDV','entry':en,'w':stdv_frac,'sl':ssl,'tp':target,'closed':False,'pnl':0.0})
                        active['stdv_added']=True

            if all(z['closed'] for z in active['legs']):
                _finish(active, rows); active=None; continue

        if active is None and len(bars)>=55 and last_signal_bucket!=bkt:
            g=build_geom(bars)
            if g:
                side,_,_,_,en0,sl,tp1,tp2,tp3=g; rg,a14=regime(bars)
                if a14:
                    ...
                    if pd_ok and abs(en-en0)<=0.35*a14:
                        w1,w2,w3=weights(rg)
                        third_kind='F3' if mode=='CORE' else 'ATR'
                        third_tp=tp3 if mode=='CORE' else None
                        active={
                            'side':side,'avg_entry':en,'mfe':0.0,'mae':0.0,
                            'pyr_added':False,'stdv_added':False,
                            'legs':[
                                {'kind':'F1','entry':en,'w':w1,'sl':sl,'tp':tp1,'closed':False,'pnl':0.0},
                                {'kind':'F2','entry':en,'w':w2,'sl':sl,'tp':tp2,'closed':False,'pnl':0.0},
                                {'kind':third_kind,'entry':en,'w':w3,'sl':sl,'tp':third_tp,'closed':False,'pnl':0.0},
                            ]
                        }
                        last_signal_bucket=bkt

    if active is not None and last_t is not None:
        side=active['side']; x=px_exit(last_t,side)
        for leg in active['legs']:
            if not leg['closed']:
                leg['pnl']=(x-leg['entry'])*side*leg['w']; leg['closed']=True
        _finish(active, rows, eod=True)
    return rows
...
def main():
    ...
    for mode in ['CORE','ATR','PYR','STDV']:
        ...
        for tar,sz,stp in params:
            rows=simulate(ticks,split,len(ticks),mode,tar,sz,stp)
            s=summarize(rows); s.update({'mode':mode,'stdv_target':tar,'stdv_frac':sz,'stdv_stop':stp}); grid.append(s)
    grid.sort(key=lambda x:(x['expectancy']>0,x['PF'],x['expectancy'],-x['maxDD_pct']),reverse=True)
    out={'experiment_id':a.experiment_id,'source':'AMOS_Fib_v0_30_STDV_LocalOpt.mq5','execution':'raw Bid/Ask tick-by-tick; completed M5 bars only for source-parity indicators/swing geometry','split':'first 40% warmup/train excluded; final 60% chronological OOS','no_ohlc_execution':True,'pd_array_note':'source placeholder reproduced as EMA9-distance gate (<=1.5 ATR effective bound); no invented OB/FVG/BPR logic','research_reference':{'N':390,'WR_pct':66.923,'PF':8.012,'DD_pct':4.0,'Monthly21_pct':99.467},'top20':grid[:20],'all':grid,'verification_level':'RAW_BIDASK_SOURCE_PARITY_CANDIDATE_V2'}
    d=Path('results/amos-fib-v030-rawtick')/a.experiment_id; d.mkdir(parents=True,exist_ok=True)
    (d/'result.json').write_text(json.dumps(out,indent=2)); print(json.dumps(out,indent=2))
