...
class Cfg(StrategyConfig,frozen=True):
 ...
 risk_mode:str='none'; dd_soft_pct:float=10.0; dd_barrier_k:float=1.5; min_add_scale:float=0.25; cvar_alpha:float=0.95; cvar_window:int=400; cvar_horizon:int=20; cvar_budget_pct:float=2.0; risk_start_layer:int=1
...
def fix_sizes(ticks):
 ...
 return out
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--catalog',required=True);ap.add_argument('--experiment-id',required=True);ap.add_argument('--base-lot',type=float,default=.30);ap.add_argument('--direction-sign',type=int,default=1);ap.add_argument('--session-start',type=int,default=7);ap.add_argument('--session-end',type=int,default=17);ap.add_argument('--basket-offset',type=float,default=1.20);ap.add_argument('--max-layers',type=int,default=10);ap.add_argument('--entry-mode',choices=['proxy','ticksmoother'],default='proxy');ap.add_argument('--ts-ticks-per-bar',type=int,default=3);ap.add_argument('--ts-fast',type=int,default=3);ap.add_argument('--ts-slow',type=int,default=5);ap.add_argument('--ts-conf1',type=int,default=8);ap.add_argument('--ts-conf2',type=int,default=13);ap.add_argument('--ts-cross-only',action='store_true');ap.add_argument('--ts-use-macd',action='store_true');ap.add_argument('--ts-no-conf',action='store_true');ap.add_argument('--risk-mode',choices=['none','barrier','cvar','hybrid'],default='none');ap.add_argument('--dd-soft-pct',type=float,default=10.0);ap.add_argument('--dd-barrier-k',type=float,default=1.5);ap.add_argument('--min-add-scale',type=float,default=0.25);ap.add_argument('--cvar-budget-pct',type=float,default=2.0);ap.add_argument('--risk-start-layer',type=int,default=1);ap.add_argument('--raw-bidask-only',action='store_true');a=ap.parse_args()
