...
class Cfg(StrategyConfig, frozen=True):
    ...
    # Utility weights: profit + DD/margin/inventory relief - future edge loss
    w_profit: float = 1.00
...
class ABStrategy(Strategy):
    def __init__(self, config: Cfg):
        ...
        self.cluster_peak={}
        self.last_profit_signature=None
        self.recluster_changes=0
        self.exit_counts=defaultdict(int)
        self.exit_pnl=defaultdict(float)
        self.inventory_release=defaultdict(int)
    ...
    def on_bar(self, bar:NBar):
        ...
        self.logic.on_bar(b)
        self.regime_counts[self.logic.regime.name]+=1
    ...
    def _enter(self,side:int,px:float,n:int):
        ...
        for _ in range(n):
            ...
            self.next_id+=1; self.pos[p.id]=p
    ...
    def _candidate_pool(self,bid,ask):
        ...
        # Mix winners and losers so profitable winners can carry selected losers out.
        ...
        # Fill remaining slots by absolute economic relevance.
        used={r[0] for r in pool}
        remain=sorted([r for r in rows if r[0] not in used],key=lambda r:abs(r[2]),reverse=True)
        pool=(pool+remain)[:self.config.cluster_candidate_pool]
        return pool

    def _future_edge_penalty(self, subset):
        # Do not eagerly kill high-score profitable positions which may become runners.
        pen=0.0
    ...
    def _cluster_utility(self, subset):
        ...
        n=len(subset)
        return (self.config.w_profit*net
                + self.config.w_dd_release*losing_released
                + self.config.w_inventory_release*n
    ...
    def _best_profit_cluster(self,bid,ask):
        ...
        best=None
        max_k=min(self.config.cluster_max_size,len(pool))
        for k in range(self.config.cluster_min_size,max_k+1):
            for sub in itertools.combinations(pool,k):
                ...
                # reward removing at least one loser while remaining profitable
    ...
    def _best_economic_be_cluster(self,bid,ask):
        ...
        for k in range(self.config.cluster_min_size,max_k+1):
            for sub in itertools.combinations(pool,k):
                net=sum(x[2] for x in sub)
                if net < self.config.economic_be_buffer: continue
                if not any(x[2]<0 for x in sub): continue
                losing_released=sum(-x[2] for x in sub if x[2]<0)
                # Economic BE prioritizes debt/inventory release over raw profit.
                utility=net + 0.75*losing_released + 0.20*len(sub) - self._future_edge_penalty(sub)
                if best is None or utility>best[0]: best=(utility,sub,net)
        return best

    def _trail_signature(self,subset):
        return tuple(sorted(x[0] for x in subset))
    ...
    def _try_profit_harvest(self,bid,ask):
        ...
        reason=None
        if net>=self.config.profit_buffer: reason='PROFIT_HARVEST'
    ...
    def _try_economic_be(self,bid,ask):
        # Only needed when inventory is building or profit harvest found nothing.
    ...
    def _try_defense(self,bid,ask):
        # Defense is last resort, not normal cluster monetization.
        if len(self.pos)<self.config.inventory_hard_cap:return False
        candidates=[]
        desired=self.logic.desired_entry_side()
        desired_side=1 if desired==Side.LONG else (-1 if desired==Side.SHORT else 0)
        for pid,p in self.pos.items():
            pnl=self._pnl_pos(p,bid,ask); age=self.bar_i-p.opened_bar
            regime_conflict=(desired_side!=0 and p.side!=desired_side)
            if pnl<=-self.config.defense_loss and age>=self.config.defense_min_age and regime_conflict:
                candidates.append((pnl,pid))
        if not candidates:return False
        candidates.sort()  # worst first
        ids=[pid for _,pid in candidates[:self.config.cluster_max_size]]
    ...
    def _evaluate_cluster(self,bid,ask):
        # Dynamic re-clustering: every tick searches for the most monetizable subset.
        # Priority: positive harvest -> economic BE/debt release -> defense.
        # Repeat a few times so several profitable small clusters can be harvested on one tick.
        for _ in range(3):
            ...
            self._try_defense(bid,ask)
            break

        # Age-based cleanup only at extreme age; do not use ordinary time-exit as normal loss realization.
        stale=[pid for pid,p in self.pos.items() if self.bar_i-p.opened_bar>=self.config.max_hold_bars*2]
        if stale:
            cp,n=self._close_ids(stale[:self.config.cluster_max_size],bid,ask,'STALE_RELEASE')
            self.cluster_cycles.append({'ids':stale[:self.config.cluster_max_size],'pnl':cp,'n':n,'reason':'STALE_RELEASE','utility':cp})
...
def run_cell(catalog_path,mode,tf,experiment_id):
    ...
    obj={'verification_level':'NAUTILUS_RAW_BIDASK_AE_DYNAMIC_PROFIT_CLUSTER_V2','engine':'NautilusTrader BacktestEngine','nautilus_version':getattr(nautilus_trader,'__version__','unknown'),'raw_ticks':len(ticks),'ohlc_resample_used':False,'signal_bars':'Nautilus INTERNAL BID bars from Raw QuoteTicks','execution':'virtual micro-position ledger marked on raw Bid/Ask; raw spread included; native venue used for event engine/time ordering','tf_minutes':tf,**st.summary()}
