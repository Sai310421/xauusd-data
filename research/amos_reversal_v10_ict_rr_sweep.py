...
def simulate(t,z,cands,rr):
    ...
    for c in cands:
        ...
        side=int(c['side']); entry=float(ask[k] if side==1 else bid[k])
        av=float(z.atr.iloc[c['disp_i']]); sweep=float(c['extreme'])
        # Production-oriented structural stop: sweep extreme, but never tighter than 0.50 ATR from executable entry.
        minrisk=.50*av
        risk=max(abs(entry-sweep),minrisk)
        stop=entry-side*risk; target=entry+side*rr*risk
        # ICT high-R needs room to resolve; 120m fixed horizon keeps RR variants comparable.
        end=min(int(np.searchsorted(tv,(et+pd.Timedelta(minutes=120)).value,side='right')),len(t))
        if end<=k+1:continue
        result='TIME'; xp=None; xt=None; rval=None
        ...
        out.append(m.Trade(f'V10_RR{rr:g}',TF,MODE,side,str(et),str(xt),entry,stop,target,xp,float(rval),result,f'POI_{c["poi_type"]}',float(c['score']),0.,0.))
        last=xt.value
    return out
...
def main():
    src=m.resolve_source('XAUUSD','dukascopy_raw');t=m.load_ticks(src);z=m.feat(m.bars(t,TF));c=v8.detect_candidates(z)
    out=Path(os.environ.get('AMOS_V10_OUT','results/amos-v10-ict-rr-sweep'));out.mkdir(parents=True,exist_ok=True)
    rows=[]
    for rr in RRS:
        tr=simulate(t,z,c,rr); met=metrics(tr);met['RR_target']=rr;rows.append(met)
        pd.DataFrame([asdict(q) for q in tr]).to_csv(out/f'RR_{rr:g}_trades.csv',index=False)
        (out/f'RR_{rr:g}_metrics.json').write_text(json.dumps(met,indent=2),encoding='utf-8')
    pd.DataFrame(rows).to_csv(out/'summary.csv',index=False);(out/'summary.json').write_text(json.dumps(rows,indent=2),encoding='utf-8')
    manifest={'purpose':'ICT high-RR sweep on verified S3 CURRENT direction','symbol':'XAUUSD','tf':TF,'mode':MODE,'rr_targets':RRS,'entry':'actual FVG/IFVG/BPR first raw-tick touch','execution':'raw bid/ask','stop':'sweep extreme with minimum 0.50 ATR','horizon_minutes':120,'verification':'RAW_DUKASCOPY_BIDASK','note':'fixed RR sweep only; DOL and 50/30/20 runner require separately defined liquidity targets'}
    (out/'manifest.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8');print(json.dumps(rows,indent=2))
