...
def load(c):
    cat=ParquetDataCatalog(c);inst=next(x for x in cat.instruments() if x.id.symbol.value.replace('/','')=='XAUUSD')
    q=cat.query(data_cls=QuoteTick,identifiers=[inst.id.value]);n=len(q)
    t=np.fromiter((int(z.ts_event) for z in q),np.int64,count=n)
    b=np.fromiter((f(z.bid_price) for z in q),float,count=n);a=np.fromiter((f(z.ask_price) for z in q),float,count=n)
    return t,b,a
...
def mk(t,b,a,m):
    d=pd.DataFrame({'t':pd.to_datetime(t,unit='ns',utc=True),'x':(b+a)/2}).set_index('t')
    z=d.x.resample(f'{m}min',label='left',closed='left').ohlc().dropna();pc=z.close.shift()
    tr=pd.concat([z.high-z.low,(z.high-pc).abs(),(z.low-pc).abs()],axis=1).max(axis=1)
    z['atr']=tr.rolling(14).mean();up=z.high.diff();dn=-z.low.diff();p=np.where((up>dn)&(up>0),up,0.);mm=np.where((dn>up)&(dn>0),dn,0.)
    aa=tr.rolling(14).sum();pdi=100*pd.Series(p,index=z.index).rolling(14).sum()/(aa+1e-12);mdi=100*pd.Series(mm,index=z.index).rolling(14).sum()/(aa+1e-12)
    z['adx']=(100*(pdi-mdi).abs()/(pdi+mdi+1e-12)).rolling(14).mean();chg=z.close.diff().abs();z['er']=z.close.diff(10).abs()/(chg.rolling(10).sum()+1e-12)
    z['sl']=(z.close-z.close.shift(3))/(z.atr+1e-12);z['bbw']=4*z.close.rolling(20).std()/(z.close.abs()+1e-12);z['bmed']=z.bbw.rolling(20).median()
    z['rh']=z.high.shift(1).rolling(20).max();z['rl']=z.low.shift(1).rolling(20).min();return z

def prep(t,b,a):
    x=mk(t,b,a,5);y=mk(t,b,a,15)
    m5={int(k.value):tuple(float(getattr(r,n)) for n in ['adx','er','sl','bbw','bmed','rh','rl','atr']) for k,r in x.iterrows()}
    m15={int(k.value):tuple(float(getattr(r,n)) for n in ['adx','er','sl','bbw','bmed']) for k,r in y.iterrows()};return m5,m15
...
def met(p,mdd):
    p=np.array(p,float);gp=p[p>0].sum() if len(p) else 0.;gl=-p[p<0].sum() if len(p) else 0.
    return {'N':len(p),'WR_pct':100*(p>0).sum()/max(1,len(p)),'PF':gp/gl if gl else(999. if gp else 0.),'pnl':float(p.sum()),'gross_win':float(gp),'gross_loss':float(gl),'max_DD_pct':mdd}
...
def run(t,bid,ask,bias,m5,m15,variant):
    ...
    sweep_side=0;sweep_ext=None;sweep_rh=sweep_rl=sweep_atr=None;sweep_t=0
    ...
    for i,t0 in enumerate(t):
        ...
        if reg!='RANGE' or rr is None:
            ...
            if np.isfinite(w) and np.isfinite(atr) and w>max(.8,2*atr):
                sweep_min=max(.03,(.04 if variant=='SWEEP_STRICT' else .02)*atr)
                reclaim=max(.05,(.10 if variant=='SWEEP_STRICT' else .06)*atr)
                expiry=3*M5 if variant=='SWEEP_STRICT' else 5*M5
                # sweep below range low, then reclaim above low
                if sweep_side==0:
                    if mid<=rl-sweep_min:
                        sweep_side=1;sweep_ext=mid;sweep_rh=rh;sweep_rl=rl;sweep_atr=atr;sweep_t=tt
                    elif mid>=rh+sweep_min:
                        sweep_side=-1;sweep_ext=mid;sweep_rh=rh;sweep_rl=rl;sweep_atr=atr;sweep_t=tt
                elif tt-sweep_t>expiry:
                    sweep_side=0;sweep_ext=None
                elif sweep_side==1:
                    sweep_ext=min(sweep_ext,mid)
                    if mid>=sweep_rl+reclaim:
                        side=1;eng='RANGE';entries=[a];startreg='RANGE';starts['RANGE']+=1
                        target=sweep_rl+(0.50 if variant=='SWEEP_STRICT' else 0.42)*(sweep_rh-sweep_rl)
                        stop=sweep_ext-max(.08,(.08 if variant=='SWEEP_STRICT' else .05)*sweep_atr);sweep_side=0
                elif sweep_side==-1:
                    sweep_ext=max(sweep_ext,mid)
                    if mid<=sweep_rh-reclaim:
                        side=-1;eng='RANGE';entries=[b];startreg='RANGE';starts['RANGE']+=1
                        target=sweep_rh-(0.50 if variant=='SWEEP_STRICT' else 0.42)*(sweep_rh-sweep_rl)
                        stop=sweep_ext+max(.08,(.08 if variant=='SWEEP_STRICT' else .05)*sweep_atr);sweep_side=0
...
def main():
    ...
    for v in ['TREND_ONLY','SWEEP_RECLAIM','SWEEP_STRICT']:
