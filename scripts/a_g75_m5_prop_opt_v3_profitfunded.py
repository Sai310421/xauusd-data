...
O=Path('bt_results/a_g75_m5_prop_opt_v3_profitfunded'); O.mkdir(parents=True,exist_ok=True)
P=.1
...
def run(trigger=.08,add=.025,rev=.20,maxadds=10,g2=60,g3=130,base_lot=.0011,add_factor=1.5):
    ...
    for i in range(10,N):
            ...
            if ex is None and pos['a']:
                if pos['d']==1:
                    ...
                    while pos['n']<maxadds and x>=pos['la']+add:
                        pos['la']+=add
                        # Profit-funded chase: base entry risk is unchanged; only favorable-direction add size is amplified.
                        v=base_lot*add_factor
                        pos['e'].append((pos['la'],v)); pos['n']+=1; adds+=1
        ...
        if pos is None and all(np.isfinite([r1[i-1],r2[i-1],s1[i-1],s2[i-1]])) and r1[i-1]>s1[i-1]:
            k=K((r1[i-1]-s1[i-1])/P); b=(math.sqrt(r1[i-1])-k)**2; s=(math.sqrt(s1[i-1])+k)**2
            ods=[(1,b),(1,b-g2*P),(1,b-(g2+g3)*P),(-1,s),(-1,s+g2*P),(-1,s+(g2+g3)*P)]
            v=[]
            for d,p in ods:
            ...
            if len(ds)==1:
                d=next(iter(ds)); es=[(p,base_lot) for dd,p in v if dd==d]; base+=len(es)
                pos={'d':d,'e':es,'a':0,'pk':C[i],'la':sum(p for p,v in es)/len(es),'n':0}
    if not tr: return None
    t=pd.DataFrame(tr,columns=['p','n','r','date']); gp=t[t.p>0].p.sum(); gl=-t[t.p<0].p.sum(); final=eq
    biz=len(set(d for d in dates if pd.Timestamp(d).weekday()<5)); mo=((final/1000)**(21/biz)-1)*100 if final>0 else -100
    return dict(N=len(t),WR=(t.p>0).mean()*100,PF=gp/gl if gl else 9999,Net=final-1000,Final=final,BaseEntries=base,G75Adds=adds,AvgAdds=t.n.mean(),MaxDD_float=max_float_dd,MaxDailyDD=max_daily_dd,Month21_pct=mo,BusinessDays=biz)

rows=[]
for trigger in [.06,.08,.10]:
  for add in [.020,.025,.030]:
   for rev in [.15,.20]:
    for ma in [10,12]:
     for lot in [.0010,.0011,.0012,.0013]:
      for af in [1.0,1.25,1.5,1.75,2.0]:
       rr=run(trigger,add,rev,ma,60,130,lot,af)
       rows.append(dict(trigger=trigger,add=add,reversal=rev,maxadds=ma,g2=60,g3=130,base_lot=lot,add_factor=af,**rr))
out=pd.DataFrame(rows); out.to_csv(O/'all_candidates.csv',index=False)
prop=out[(out.MaxDD_float<=5.0)&(out.MaxDailyDD<=3.5)].sort_values(['Month21_pct','PF'],ascending=False); prop.to_csv(O/'prop_pass.csv',index=False)
target=prop[prop.Month21_pct>=50].copy(); target.to_csv(O/'target_50.csv',index=False)
summary={'rows':N,'start':str(df.datetime.min()),'end':str(df.datetime.max()),'tested':len(out),'prop_pass':len(prop),'target50':len(target),'top':prop.head(30).to_dict('records')}
