...
def entry_index(stage,c,tv,bid,ask):
    ...
    # Keep original signal-side POI touch clock fixed. Direction transform occurs only after touch is known.
    return v8.first_touch(tv,bid,ask,st.value,en.value,c['side'],lo,hi)

def simulate_one(t,z,cands,stage,variant):
    tv=epoch_ns(t['time']); bid=t.bid.to_numpy(float,copy=False); ask=t.ask.to_numpy(float,copy=False); mid=(bid+ask)/2.0
    ...
    for c in cands:
        ...
        av=float(z.atr.iloc[ref_i]); risk=0.50*av
        ...
        if exit_px is None:
            j=end-1; exit_px=float(mid[j]); exit_t=pd.Timestamp(t.time.iloc[j]); rval=side*(exit_px-entry)/risk
        tr=m.Trade(f'V9B_{variant}',TF,MODE,side,str(touch),str(exit_t),entry,stop,target,exit_px,float(rval),result,
...
def main():
    ...
    out=Path(os.environ.get('AMOS_V9B_OUT','results/amos-v9b-polarity-mid')); out.mkdir(parents=True,exist_ok=True)
    rows=[]
    for stage in STAGES:
    ...
    manifest={'purpose':'Pure direction polarity isolation before costs','symbol':'XAUUSD','timeframe':'M1','mode':'STANDARD','rr':RR,
      'raw_tick_source':str(src),'verification_level':'RAW_DUKASCOPY_BIDASK_SOURCE_MID_EVAL','stages':STAGES,'variants':VARIANTS,
      'entry_clock':'identical per stage before polarity transform','evaluation_price':'mid=(bid+ask)/2','risk_model':'symmetric 0.50 ATR',
      'costs':'excluded intentionally for polarity diagnosis','horizon_minutes':20,'bars_label':'right','bars_closed':'right','rows':len(t)}
