...
# Raw Bid/Ask execution. Bars are derived only from raw ticks for signal state.
@dataclass
class Trade:
...
def adx_proxy(b,n=14):
    # deterministic regime proxy for public reproduction; MT5 confirmation uses native ADX
...
def zigzag_points(b,depth=12,deviation_points=5,point=0.01,backstep=3):
    # MetaQuotes-style deterministic approximation: local Depth extrema, de-duplicated/backstepped.
    pts=[]; last_hi_i=-10**9; last_lo_i=-10**9
    for i in range(depth,len(b)-2):
        lo=float(b.low.iloc[i]); hi=float(b.high.iloc[i])
        w=b.iloc[i-depth+1:i+1]
        if lo <= float(w.low.min()) + 1e-12:
            if i-last_lo_i>backstep or (pts and lo < pts[-1][1]):
                pts.append((i,lo,'L')); last_lo_i=i
        if hi >= float(w.high.max()) - 1e-12:
            if i-last_hi_i>backstep or (pts and hi > pts[-1][1]):
                pts.append((i,hi,'H')); last_hi_i=i
    pts.sort(key=lambda z:z[0])
    # Alternate H/L retaining more extreme same-type pivots.
    out=[]
    for p in pts:
...
def shifted_hour(ts):
    # uploaded defaults GMT=2, GMTS=3 make ShiftedHour equal server/chart hour in EA.
    return int(ts.hour)
...
def build_entries(ticks):
    ...
    # EA-parity direction: at each current scan time, compare COMPLETED historical
    # ZigZag pivots with the CURRENT day's running barrier range. Pending stop
    # levels are active only inside the permitted hour because the EA cancels
    # pendings when leaving allowed hours.
    for layer,b,pts,cap in [('A',h1,pA,5),('B',m15,pB,3)]:
        ptimes=[b.time.iloc[i] for i,_,_ in pts]
        for si in range(30,len(b)-1):
            ts=b.time.iloc[si]
            if shifted_hour(ts) not in trade_hours: continue
            day=ts.date()
            db=m15[(m15.time.dt.date==day) & (m15.time<=ts)]
            if db.empty: continue
            dhi=float(db.high.max()); dlo=float(db.low.min())
            ...
            # last 600 completed bars, same contract as uploaded EA LookbackBars.
            lo_i=max(0,si-600)
            cands=[]
            for (pi,price,typ),pt in zip(pts,ptimes):
                if pi<lo_i or pi>=si: continue
                if typ=='H' and price>dhi+mind: cands.append((abs(price-dhi),1,price))
                elif typ=='L' and price<dlo-mind: cands.append((abs(dlo-price),-1,price))
            # nearest outside pivots per side approximate LevelsPerSide A/B.
            for side in (1,-1):
                cc=sorted([x for x in cands if x[1]==side],key=lambda z:z[0])[:cap]
                for _,_,price in cc:
                    stop_level=price-off if side==1 else price+off
                    key=(day,round(stop_level,3),side,layer)
                    if key in seen: continue
                    seen.add(key)
                    j0=np.searchsorted(ticks.time.values,np.datetime64(ts.to_datetime64()),side='right')
                    hour_end=ts.floor('h')+pd.Timedelta(hours=1)
                    for k in range(j0,len(ticks)):
                        q=ticks.iloc[k]
                        if q.time>=hour_end: break
                        if (side==1 and q.ask>=stop_level) or (side==-1 and q.bid<=stop_level):
                            ...
                            sl=max(min(av*1.2,12.0),4.0)
                            tp=max(min(av*2.4,30.0),9.0)*tp_mult
                            entries.append(dict(time=q.time,side=side,entry=entry,atr=av,regime=regime,sl=sl,tp=tp,layer=layer,idx=k))
                            break
...
def simulate_native(ticks,e):
    ...
    sl=entry-side*sl0; tp=entry+side*tpdist; risk=sl0; best=entry; mae=mfe=0.0
    # native BE: trigger 1.2 USD * vol approximation omitted -> baseline 1.2, lock 0.2
    for k in range(idx+1,len(ticks)):
        q=ticks.iloc[k]; px=float(q.bid if side==1 else q.ask)
        ex=side*(px-entry); mfe=max(mfe,ex); mae=min(mae,ex)
        if ex>=1.2:
            be=entry+side*0.2
            sl=max(sl,be) if side==1 else min(sl,be)
        # Native M1-extreme trail is approximated conservatively with 3.0 USD offset after +2.0.
        if ex>=2.0:
            best=max(best,px) if side==1 else min(best,px)
            nsl=best-side*3.0
            sl=max(sl,nsl) if side==1 else min(sl,nsl)
        stop_hit=(px<=sl) if side==1 else (px>=sl)
        tp_hit=(px>=tp) if side==1 else (px<=tp)
...
def simulate_adaptive(ticks,e):
    # Public/default-feature reproduction, not EX5 byte parity.
    side=e['side']; entry=e['entry']; idx=e['idx']; av=e['atr']; risk=max(1.5*av,0.01)
    sl=entry-side*risk; mae=mfe=0.0
    # Defaults publicly confirmed: 40/30/remainder. Target distances are not public, so use 1R/2R/runner as neutral ATR-normalized test.
    t1=entry+side*1.0*av; t2=entry+side*2.0*av
    rem=1.0; pnl=0.0; p1=p2=False; best=entry
    for k in range(idx+1,len(ticks)):
        ...
        if not p1 and ((px>=t1) if side==1 else (px<=t1)):
            pnl += 0.40*side*(px-entry); rem-=0.40; p1=True
            # move BE after first target, include small lock to represent spread-safe BE
            sl=entry+side*float(q.spread)
        if p1 and not p2 and ((px>=t2) if side==1 else (px<=t2)):
            pnl += 0.30*side*(px-entry); rem-=0.30; p2=True
        if p2:
            best=max(best,px) if side==1 else min(best,px)
            # step trail: 1 ATR behind best, only meaningful improvements
            nsl=best-side*av
            if side==1 and nsl>sl+0.2*av: sl=nsl
            if side==-1 and nsl<sl-0.2*av: sl=nsl
        if (px<=sl) if side==1 else (px>=sl):
