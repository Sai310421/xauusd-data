...
from research.armada_nautilus_raw_cell import _ensure_l1_liquidity, _status_counts, _tick_diag
...
class EVG75NativeStrategy(Strategy):
    ...
    def _submit(self, side):
        ...
        order=self.order_factory.market(
            ...
            order_side=side,
            quantity=inst.make_qty(self.config.trade_size),
        )
    ...
    def on_quote_tick(self,tick:QuoteTick):
        ...
        if self.layers<self.config.max_layers and ((self.side>0 and px>=target) or (self.side<0 and px<=target)):
            self._submit(OrderSide.BUY if self.side>0 else OrderSide.SELL)
            self.adds+=1; self.layers+=1; self.last_add=target
...
def metrics(report, initial=1000.0, days=30):
    ...
    net=float(a.sum()); mdd_pct=mdd/peak*100 if peak>0 else 0.0
...
def main():
    ...
    ap.add_argument('--trigger',type=float,required=True); ap.add_argument('--add',type=float,required=True)
    ap.add_argument('--reversal',type=float,required=True); ap.add_argument('--trade-size',default='1')
    ...
    cp=Path(a.catalog); man=json.loads((cp/'catalog_manifest.json').read_text()); days=int(man['days'])
    cat=ParquetDataCatalog(str(cp)); inst=next(x for x in cat.instruments() if x.id.symbol.value.replace('/','')=='XAUUSD')
    raw_ticks=list(cat.query(data_cls=QuoteTick, identifiers=[inst.id.value]))
    if not raw_ticks: raise SystemExit('NO_RAW_TICKS')
    ...
    exec_venue=inst.id.venue
    engine.add_venue(
        venue=exec_venue, oms_type=OmsType.NETTING, account_type=AccountType.MARGIN,
    ...
    summary={
      'verification_level':'NAUTILUS_NATIVE_ORDER_RAW_BIDASK_L1_SIZEFIX',
      'engine':'NautilusTrader BacktestEngine','nautilus_version':getattr(nautilus_trader,'__version__','unknown'),
      'params':{'trigger':a.trigger,'add':a.add,'reversal':a.reversal,'trade_size':a.trade_size},
      'raw_ticks':len(raw_ticks),'execution_ticks':len(ticks),'raw_zero_size_quotes_replaced':replaced,
      'signals':{'entries':strat.entries,'adds':strat.adds,'exits':strat.exits},
      'native_orders':int(len(orders)),'native_order_status_counts':_status_counts(orders),
      'native_fills':int(len(fills)),'native_positions':int(len(report)),
      'native_fill_gate_pass':bool(len(fills)>0),
      'metrics':m,'execution_venue':str(exec_venue),'book_type':'L1_MBP',
      'first_raw_quote':_tick_diag(raw_ticks[0]),'first_execution_quote':_tick_diag(ticks[0]),
      'execution':'MARKET orders on raw Bid/Ask; nonpositive L1 sizes replaced with Quantity(1) only for matching; observed spread native; commission/slippage not yet added',
      'period':man}
    if len(fills)==0: raise SystemExit('NATIVE_FILL_GATE_FAILED')
    (out/'summary.json').write_text(json.dumps(summary,indent=2,default=str)); print(json.dumps(summary,default=str))
    engine.dispose()
