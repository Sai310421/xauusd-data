...
class Cfg(StrategyConfig,frozen=True):
 ...
 session_start_hour:int=7; session_end_hour:int=17; entry_move:float=1.80; cooldown_seconds:int=45; emergency_cooldown_seconds:int=45; reset_signal_on_emergency:bool=False; conditional_stop:bool=False; stop_gate_distance:float=4.5; hard_stop_distance:float=5.5; stop_vel_window:int=24; stop_adverse_ratio:float=0.62; direction_sign:int=1
...
class TickScalperCandidate(Strategy):
 ...
 def on_quote_tick(self,t):
  ...
  if len(self.entries)>=self.config.max_layers:
   ...
   if self.config.conditional_stop:
    hard=adverse2>=self.config.hard_stop_distance
    gate=adverse2>=self.config.stop_gate_distance
    vals=list(self.recent_mid_deltas)[-self.config.stop_vel_window:]
    if vals:
     adverse_n=sum(1 for d in vals if d*s<0)
     adverse_ratio=adverse_n/len(vals)
    else: adverse_ratio=0.0
    ts_against=(self.ts_prev_signal!=0 and self.ts_prev_signal==-s)
    if hard or (gate and adverse_ratio>=self.config.stop_adverse_ratio and ts_against):
     self._close('EMERGENCY');return
   elif adverse2>=self.config.emergency_distance:
    self._close('EMERGENCY');return
 def on_stop(self):
...
def fix_sizes(ticks):
 ...
 return out
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--catalog',required=True);ap.add_argument('--experiment-id',required=True);ap.add_argument('--base-lot',type=float,default=.30);ap.add_argument('--direction-sign',type=int,default=1);ap.add_argument('--session-start',type=int,default=7);ap.add_argument('--session-end',type=int,default=17);ap.add_argument('--basket-offset',type=float,default=1.20);ap.add_argument('--max-layers',type=int,default=10);ap.add_argument('--emergency-distance',type=float,default=3.80);ap.add_argument('--emergency-cooldown',type=int,default=45);ap.add_argument('--reset-signal-on-emergency',action='store_true');ap.add_argument('--conditional-stop',action='store_true');ap.add_argument('--stop-gate-distance',type=float,default=4.5);ap.add_argument('--hard-stop-distance',type=float,default=5.5);ap.add_argument('--stop-vel-window',type=int,default=24);ap.add_argument('--stop-adverse-ratio',type=float,default=0.62);ap.add_argument('--entry-mode',choices=['proxy','ticksmoother'],default='proxy');ap.add_argument('--ts-ticks-per-bar',type=int,default=3);ap.add_argument('--ts-fast',type=int,default=3);ap.add_argument('--ts-slow',type=int,default=5);ap.add_argument('--ts-conf1',type=int,default=8);ap.add_argument('--ts-conf2',type=int,default=13);ap.add_argument('--ts-cross-only',action='store_true');ap.add_argument('--ts-use-macd',action='store_true');ap.add_argument('--ts-no-conf',action='store_true');ap.add_argument('--risk-mode',choices=['none','barrier','cvar','hybrid','firstpass','fp_barrier','dd_adaptive','floating_governor','dd_floating_combo'],default='none');ap.add_argument('--dd-soft-pct',type=float,default=10.0);ap.add_argument('--dd-barrier-k',type=float,default=1.5);ap.add_argument('--min-add-scale',type=float,default=0.25);ap.add_argument('--cvar-budget-pct',type=float,default=2.0);ap.add_argument('--risk-start-layer',type=int,default=1);ap.add_argument('--dd-t1',type=float,default=10.0);ap.add_argument('--dd-t2',type=float,default=20.0);ap.add_argument('--dd-t3',type=float,default=30.0);ap.add_argument('--dd-s1',type=float,default=0.85);ap.add_argument('--dd-s2',type=float,default=0.65);ap.add_argument('--dd-s3',type=float,default=0.40);ap.add_argument('--fdd-t1',type=float,default=10.0);ap.add_argument('--fdd-t2',type=float,default=20.0);ap.add_argument('--fdd-t3',type=float,default=30.0);ap.add_argument('--fdd-s1',type=float,default=0.85);ap.add_argument('--fdd-s2',type=float,default=0.60);ap.add_argument('--fdd-s3',type=float,default=0.35);ap.add_argument('--fp-threshold',type=float,default=0.60);ap.add_argument('--fp-block-threshold',type=float,default=0.78);ap.add_argument('--raw-bidask-only',action='store_true');a=ap.parse_args()
