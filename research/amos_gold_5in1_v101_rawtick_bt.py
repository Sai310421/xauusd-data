...
def simulate(t,sigs,capital=1000,lot=.01):
 ...
 return tr,s
def main():
 ap=argparse.ArgumentParser();ap.add_argument("--start",default="2026-07-27");ap.add_argument("--days",type=int,default=21);ap.add_argument("--workers",type=int,default=8);ap.add_argument("--engine",default="ALL",choices=["ALL","AI","FLASH","GRAND","LIMIT","NINE"]);ap.add_argument("--cache",default=None);ap.add_argument("--out",default=str(OUT));a=ap.parse_args()
 outdir=Path(a.out); outdir.mkdir(parents=True,exist_ok=True); t=load(days(a.start,a.days),a.workers,a.cache)
 ...
 tr,s=simulate(t,sig); tr.to_csv(outdir/"trades.csv",index=False)
 by={}
 if len(tr):
 ...
 (outdir/"summary.json").write_text(json.dumps(out,indent=2)); print(json.dumps(out,indent=2))
