...
class Cfg(StrategyConfig,frozen=True):
 ...
 control_from_depth:int=7; control_min_scale:float=0.30; er_ref:float=0.33; spread_ref:float=0.70; control_strength:float=0.85; tail_window:int=64
...
class TickScalperCandidate(Strategy):
 def __init__(self,c):
  super().__init__(c);self.bid=self.ask=None;self.side=0;self.entries=[];self.trades=[];self.gw=self.gl=self.net=0.;self.eq=self.peak=1000.;self.mdd=self.max_lots=0.;self.max_layer=0;self.last_close_ns=0;self.mid_hist=[];self.control_events=0;self.control_saved_lots=0.
 ...
 def on_quote_tick(self,t):
  self.bid=float(t.bid_price.as_double());self.ask=float(t.ask_price.as_double());self.now_ns=int(t.ts_event);mid=(self.bid+self.ask)/2;self.mid_hist.append(mid);self.mid_hist=self.mid_hist[-self.config.tail_window:]
  if not self.entries:
  ...
  if adverse>=self.config.add_distance and len(self.entries)<self.config.max_layers:
   raw_lot=layer_lot(float(self.config.base_qty),len(self.entries),self.config);lot=raw_lot
   if len(self.entries)>=self.config.control_from_depth and len(self.mid_hist)>=3:
    path=sum(abs(b-a) for a,b in zip(self.mid_hist,self.mid_hist[1:]));er=abs(self.mid_hist[-1]-self.mid_hist[0])/path if path>0 else 0.;spread=self.ask-self.bid
    risk=max(er/self.config.er_ref,spread/self.config.spread_ref,1.0);depth_pressure=1.0+self.config.control_strength*(len(self.entries)-self.config.control_from_depth+1)*(risk-1.0)
    scale=max(self.config.control_min_scale,1.0/depth_pressure);lot=floor_step(raw_lot*scale);lot=max(.01,lot)
    if lot<raw_lot:self.control_events+=1;self.control_saved_lots+=raw_lot-lot
...
def fix_sizes(ticks):
 ...
 return out
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--catalog',required=True);ap.add_argument('--experiment-id',required=True);ap.add_argument('--base-lot',type=float,default=.30);ap.add_argument('--control-strength',type=float,default=.85);ap.add_argument('--control-min-scale',type=float,default=.30);ap.add_argument('--control-from-depth',type=int,default=7);ap.add_argument('--raw-bidask-only',action='store_true');a=ap.parse_args()
 ...
 st=TickScalperCandidate(Cfg(instrument_id=inst.id,base_qty=Decimal(str(a.base_lot)),control_strength=a.control_strength,control_min_scale=a.control_min_scale,control_from_depth=a.control_from_depth));eng.add_strategy(st);eng.run();fills=eng.trader.generate_order_fills_report()
 o={'verification_level':'NAUTILUS_RAW_BIDASK_CANDIDATE_NOT_REPLICA','raw_ticks':len(ticks),'native_fills':len(fills) if fills is not None else 0,'ohlc_resample_used':False,'entry_rule':'MATH_CONTROL_V2_PARAMETER_SWEEP','control_strength':a.control_strength,'control_min_scale':a.control_min_scale,'control_from_depth':a.control_from_depth,'quantity_mapping':'1.00 lot = 100 XAU units; 0.30 lot = 30 native units','instrument_size_precision':getattr(inst,'size_precision',None),**st.summary()}
