...
class Cfg(StrategyConfig,frozen=True):
 ...
 add_distance:float=3.11; single_target:float=1.20; basket_offset:float=0.876; emergency_distance:float=3.80
...
class TickScalperCandidate(Strategy):
 ...
 def on_quote_tick(self,t):
  ...
  target=self.config.single_target if len(self.entries)==1 else self.config.basket_offset
  if (mark-be)*s>=target:self._close('SINGLE' if len(self.entries)==1 else 'BASKET');return
...
def main():
 ...
 o={'verification_level':'NAUTILUS_RAW_BIDASK_CANDIDATE_NOT_REPLICA','raw_ticks':len(ticks),'native_fills':len(fills) if fills is not None else 0,'ohlc_resample_used':False,'entry_rule':'STATEMENT_CONSTRAINED_V3_SPLIT_EXIT','quantity_mapping':'1.00 lot = 100 XAU units; 0.30 lot = 30 native units','instrument_size_precision':getattr(inst,'size_precision',None),**st.summary()}
