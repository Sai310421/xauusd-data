...
class TickScalperCandidate(Strategy):
 def __init__(self,c):
  super().__init__(c);self.bid=self.ask=None;self.side=0;self.entries=[];self.trades=[];self.gw=self.gl=self.net=0.;self.eq=self.peak=1000.;self.mdd=self.max_lots=0.;self.max_layer=0;self.last_close_ns=0;self.mid_hist=[];self.risk_signals=0;self.risk_seen_depths=set();self.ticket_pnls=[];self.mode='RANGE';self.mode_counts={'TREND':0,'RANGE':0};self.entry_ers=[];self.entry_spreads=[]
 ...
 def _open(self,side):
  if len(self.mid_hist)>=3:
   net=abs(self.mid_hist[-1]-self.mid_hist[0]);path=sum(abs(b-a) for a,b in zip(self.mid_hist,self.mid_hist[1:]));er=net/path if path>0 else 0.
   self.mode='TREND' if er>=self.config.trend_er_threshold else 'RANGE';self.entry_ers.append(er);self.entry_spreads.append(self.ask-self.bid)
  else:self.mode='RANGE'
 ...
 def _close(self,reason):
  ...
  self.side=0;self.entries=[];self.mode='RANGE';self.risk_seen_depths=set();self.last_close_ns=getattr(self,'now_ns',self.last_close_ns)
 ...
 def on_quote_tick(self,t):
  self.bid=float(t.bid_price.as_double());self.ask=float(t.ask_price.as_double());self.now_ns=int(t.ts_event);mid=(self.bid+self.ask)/2;self.mid_hist.append(mid);self.mid_hist=self.mid_hist[-self.config.er_window:]
  if not self.entries:
 ...
 def summary(self):
  n=len(self.trades);wins=sum(x['pnl']>0 for x in self.trades);return {'N':n,'WR_pct':100*wins/max(n,1),'PF':self.gw/self.gl if self.gl else None,'EV':self.net/max(n,1),'Net':self.net,'Return_pct':100*self.net/1000,'MaxDD_pct':self.mdd,'max_layer':self.max_layer,'max_concurrent_lots':self.max_lots,'depth10':sum(x['depth']>=10 for x in self.trades),'risk_signals':self.risk_signals,'ticket_N':len(self.ticket_pnls),'ticket_WR_pct':100*sum(p>0 for p in self.ticket_pnls)/max(len(self.ticket_pnls),1),'ticket_PF':sum(p for p in self.ticket_pnls if p>0)/max(sum(-p for p in self.ticket_pnls if p<0),1e-12),'mode_counts':self.mode_counts,'entry_ER_p25':sorted(self.entry_ers)[int(.25*(len(self.entry_ers)-1))] if self.entry_ers else None,'entry_ER_p50':sorted(self.entry_ers)[int(.50*(len(self.entry_ers)-1))] if self.entry_ers else None,'entry_ER_p75':sorted(self.entry_ers)[int(.75*(len(self.entry_ers)-1))] if self.entry_ers else None,'entry_ER_p90':sorted(self.entry_ers)[int(.90*(len(self.entry_ers)-1))] if self.entry_ers else None,'entry_spread_p50':sorted(self.entry_spreads)[int(.50*(len(self.entry_spreads)-1))] if self.entry_spreads else None,'entry_spread_p90':sorted(self.entry_spreads)[int(.90*(len(self.entry_spreads)-1))] if self.entry_spreads else None}
def fix_sizes(ticks):
 ...
 return out
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--catalog',required=True);ap.add_argument('--experiment-id',required=True);ap.add_argument('--base-lot',type=float,default=.30);ap.add_argument('--entry-move',type=float,default=1.20);ap.add_argument('--session-start',type=int,default=7);ap.add_argument('--session-end',type=int,default=17);ap.add_argument('--cooldown',type=int,default=45);ap.add_argument('--raw-bidask-only',action='store_true');a=ap.parse_args()
 ...
 st=TickScalperCandidate(Cfg(instrument_id=inst.id,base_qty=Decimal(str(a.base_lot)),entry_move=a.entry_move,session_start_hour=a.session_start,session_end_hour=a.session_end,cooldown_seconds=a.cooldown));eng.add_strategy(st);eng.run();fills=eng.trader.generate_order_fills_report()
 o={'verification_level':'NAUTILUS_RAW_BIDASK_CANDIDATE_NOT_REPLICA','raw_ticks':len(ticks),'native_fills':len(fills) if fills is not None else 0,'ohlc_resample_used':False,'entry_rule':'PARITY_CALIBRATION_ENTRY_CADENCE','entry_move':a.entry_move,'session_start':a.session_start,'session_end':a.session_end,'cooldown':a.cooldown,'quantity_mapping':'1.00 lot = 100 XAU units; 0.30 lot = 30 native units','instrument_size_precision':getattr(inst,'size_precision',None),**st.summary()}
