...
class CausalGateCandidate(ArmadaCandidate):
    """Actual forward gate replay on the E2 baseline.

    This intentionally differs from Stage5 counterfactual post-hoc filtering: the gate is
    applied at signal time, so rejected trades alter the future position/entry path.
    """
    def __init__(self, config, min_trend_strength=0.0, min_signed_slope=-999.0, max_breakout_atr=999.0):
        super().__init__(config)
        self.min_trend_strength=float(min_trend_strength)
        self.min_signed_slope=float(min_signed_slope)
    ...
    def _signal(self):
        ...
        breakout_atr=((c[-1]-hh)/atr) if sig>0 else ((ll-c[-1])/atr)

        reasons=[]
        if trend_strength < self.min_trend_strength: reasons.append('trend')
        if signed_slope < self.min_signed_slope: reasons.append('signed_slope')
        if breakout_atr > self.max_breakout_atr: reasons.append('breakout_high')
        if reasons:
            self.filtered_signals += 1
    ...
    def summary(self):
        ...
        x.update({
            ...
            'retention_pct':100.0*self.accepted_signals/max(self.raw_signals,1),
            'gate_reasons':dict(self.gate_reasons),
            'causal_gate':{
                'min_trend_strength':self.min_trend_strength,
                'min_signed_slope':self.min_signed_slope,
...
def main():
    ...
    ap.add_argument('--min-signed-slope',type=float,default=-999.0)
    ...
    st=CausalGateCandidate(cfg,a.min_trend_strength,a.min_signed_slope,a.max_breakout_atr)
    ...
    obj={
      'verification_level':'NAUTILUS_BT_RAW_BIDASK_ARMADA_APEX_STAGE6_CAUSAL_GATE_NATIVE',
      ...
      'tf':'M15','family':'R2','base_exit':'E2: trail=1.00 protect=0.50 hold=720 stop=none',**st.summary(),
      ...
      'disclaimer':'Stage6 is causal in-sample replay of Stage5 hypotheses; OOS validation is still required.'
    }
