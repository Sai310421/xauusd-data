...
def run_combo(ticks: Iterable[QuoteTick], trigger: float, add: float, reversal: float) -> Result:
    ...
    if side!=0:
        t=list(ticks)[-1] if not isinstance(ticks,list) else ticks[-1]
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--catalog',required=True); ap.add_argument('--experiment-id',required=True); ap.add_argument('--raw-bidask-only',action='store_true'); ap.add_argument('--symbol',default='XAUUSD')
    a=ap.parse_args()
    ...
    for tr in TRIGGERS:
        for ad in ADDS:
            for rv in REVERSALS:
                r=run_combo(ticks,tr,ad,rv); rows.append(r.__dict__)
    ...
    summary={'verification_level':'RAW_BIDASK_EV_SURFACE_RESEARCH','symbol':a.symbol,'raw_ticks':len(ticks),'period':man,'grid':{'trigger':TRIGGERS,'add':ADDS,'reversal':REVERSALS,'combinations':len(rows)},'selection_rule':'n>=100, EV>0, PF>1, MaxDD<=20; rank by EV*sqrt(N)/(1+DD/5)','top25':top,'note':'This is a deterministic price-distance expectancy surface, not yet broker-reality BT. Commission/probabilistic slippage and OOS split required before promotion.'}
