...
def trade_path(ticks,entry_time,direction,atr,mode='FIXED5',boost=1.0):
    ...
    if mode=='FIXED5': ex=last
        ...
        for _,r in p.iloc[1:].iterrows():
            ...
            if fav>=trail_act*atr: trail_on=True
            trail=(best-direction*trail_act*atr) if trail_on else None
            stop_hit=(px<=sl if direction>0 else px>=sl); tp_hit=(px>=tp if direction>0 else px<=tp); trail_hit=trail_on and (px<=trail if direction>0 else px>=trail)
            if stop_hit or tp_hit or trail_hit: ex=px; break
    return entry,ex,direction*(ex-entry)*QTY*boost
...
def main():
    ...
    built={tf:swing_sweeps(core.edges(core.bars(ticks,s))) for tf,s in core.TF_SECONDS.items()}; selected=[('M1','IMBALANCE'),('M5','ABSORPTION')]; raw=[]
    for tf,edge in selected:
        ...
        for i in np.flatnonzero(sig!=0):
            ...
            if ok: raw.append({'tf':tf,'edge':edge,'direction':d,'entry_time':et,'atr':float(b.atr14.iat[i]),'liq':bool(b.bull_sweep.iat[i] if d>0 else b.bear_sweep.iat[i])})
    ...
    configs=[('BIDASK_FIXED5',False,'FIXED5'),('LIQ_BOOST_FIXED5',True,'FIXED5'),('BIDASK_TESTBOT_EXIT',False,'TESTBOT'),('BIDASK_FAST_EXIT',False,'FAST'),('LIQ_TESTBOT',True,'TESTBOT'),('LIQ_FAST',True,'FAST')]; rows=[]
    ...
    pd.DataFrame(rows).to_csv(OUT/'summary_21d.csv',index=False); (OUT/'provenance.json').write_text(json.dumps({'formula_policy':'FROZEN_BIGPLAYER_NO_INTERNAL_CHANGES','entry_supervisor':'same proxy TREND+REGIME winner from prior screen','mtf_boost':'same-minute same-direction x2.0','liquidity':'50-bar prior swing sweep: pierce then close back','execution':'raw bid/ask, 5-minute max horizon','exit_branches':{'TESTBOT':'ATR14 SL0.3 / TP0.6 / trail activation-distance0.2','FAST':'ATR14 SL1.0 / TP2.0 / trail after1.2 ATR'}},indent=2)); print(pd.DataFrame(rows).to_string(index=False))
