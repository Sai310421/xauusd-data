...
class Cfg(StrategyConfig,frozen=True):
 ...
 er_window:int=64; risk_gate_from_depth:int=7; er_tail_threshold:float=0.42; spread_tail_threshold:float=0.35; trend_er_threshold:float=0.55; trend_target:float=1.20
...
def main():
 ...
 o={'verification_level':'NAUTILUS_RAW_BIDASK_CANDIDATE_NOT_REPLICA','raw_ticks':len(ticks),'native_fills':len(fills) if fills is not None else 0,'ohlc_resample_used':False,'entry_rule':'MATH_V8_ER_DISTRIBUTION_DIAGNOSTIC','quantity_mapping':'1.00 lot = 100 XAU units; 0.30 lot = 30 native units','instrument_size_precision':getattr(inst,'size_precision',None),**st.summary()}
